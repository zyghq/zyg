...
export const inSync = restate.service({
  ...
  handlers: {
    ...
    dbtime: async (_: restate.Context) => {
...
type ThreadLabelsOrEmpty = ThreadLabels | Record<string, unknown>;

export const sync = restate.object({
  name: "sync",
  handlers: {
    ...
    addThreadLabels: async (
      ...
      const existingLabelRows = await syncdb
    ...
    deleteThreadLabels: async (
      ...
      const existingLabels = existingLabelRows[0] || { labels: {} };
      const { labels } = existingLabels as { labels: Record<string, unknown> };
