import { pgTable, varchar, timestamp, uuid, foreignKey, jsonb, unique, boolean, text } from "drizzle-orm/pg-core"
...
export const workspace = pgTable("workspace", {
	...
	syncedAt: timestamp("synced_at", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
});
...
export const customer = pgTable("customer", {
	...
	unique("customer_workspace_id_external_id_key").on(table.workspaceId, table.externalId),
	unique("customer_workspace_id_email_key").on(table.workspaceId, table.email),
	unique("customer_workspace_id_phone_key").on(table.workspaceId, table.phone),
]);
...
export const thread = pgTable("thread", {
	...
	statusChangedAt: timestamp("status_changed_at", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	statusChangedById: varchar("status_changed_by_id", { length: 255 }).notNull(),
